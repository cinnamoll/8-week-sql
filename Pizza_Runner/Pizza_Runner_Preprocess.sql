...
UPDATE runner_orders SET duration = TRIM(REGEXP_REPLACE(duration, '[^0-9]', ''));
ALTER TABLE runner_orders
MODIFY COLUMN duration DECIMAL(10, 0);

DROP TABLE IF EXISTS pizza_recipes_split;
CREATE TABLE pizza_recipes_split (
    pizza_id INTEGER,
    pizza_toppings INTEGER
);

INSERT INTO pizza_recipes_split (pizza_id, pizza_toppings)
WITH RECURSIVE split_val AS (
    SELECT
        pizza_id,
        CAST(SUBSTRING_INDEX(toppings, ',', 1) AS UNSIGNED) AS split_value,
        IF(LOCATE(',', toppings) > 0,
               TRIM(SUBSTRING(toppings, LOCATE(',', toppings) + 1)),
               NULL
        ) AS remain
    FROM pizza_recipes
    UNION ALL
    SELECT
        pizza_id,
        CAST(SUBSTRING_INDEX(remain, ',', 1) AS UNSIGNED) AS split_value,
        IF(
                LOCATE(',', remain) > 0,
                TRIM(SUBSTRING(remain, LOCATE(',', remain) + 1)),
                NULL
        )
    FROM split_val
